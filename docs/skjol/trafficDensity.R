...
mtq <- st_read(dsn = here::here("docs/skjol/vegr.gpkg/vegagerdin.shp"), quiet = TRUE)
mtq <- mtq[grep("F",mtq$NRVEGUR,invert = T),]
mtq <- mtq[mtq$VEGHEITI!="Kjalvegur",]
mtq$ADU <- as.numeric(mtq$ADU)
mtq$aduB <- round(mtq$ADU)
...
png(filename=here::here("docs/images/trafficDensity_BGB.png"),16,9,"cm",pointsize=6,res=900)
...
smoothLayer(
  ...
  mask = mtq, 
  legend.values.rnd = -3,
  legend.title.txt = "Meðalumferð á dag yfir árið",
